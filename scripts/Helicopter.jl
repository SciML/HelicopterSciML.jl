...
plot!(
    title = "Experimental helicopter outputs: measured angles",
    ylabel = L"angles [$^\circ$]",
    xlabel = L"time $t$ [s]",
    box = true,
)
...
plot!(
    title = "Experimental helicopter inputs: motor voltages",
    ylabel = L"$u$ [V]",
    xlabel = L"time $t$ [s]",
    box = true,
)
...
pidx = [findall(Ps_keys[i] .== keys(P0))[1] for i = 1:length(Ps_keys)]
#
xidx = collect(3:4)
#
