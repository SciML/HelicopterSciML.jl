...
model_univ = FastChain(FastDense(6, 16, tanh), FastDense(16, 2))
include("../optimization_results/neural_augmentation_results_u.jl")
...
# Array of potential basis functions
h = [cos.(u)...; sin.(u)...; u[1]^0; u[1]^2; u[2]^2; u[1]*u[2]]
basis_u = Basis(h, u)
println(basis_u)
...
Ψ = SInDy(X, DX, basis_u, λ, opt = opt, maxiter = 10000, f_target = f_target)
println(Ψ)
print_equations(Ψ)
...
function helicopter_uode(dx, x, Ps, P0, u, p, t)
    ...
    U = [u.th(t), u.psi(t)]
    K = Ψ.equations.f_(U, Ψ.parameters, t)

    #
    M11 = Ps[4]*P0.Jbc + Ps[3]*P0.m*((Ps[1]*P0.rc)^2+(Ps[2]*P0.h)^2)
    M22 =
        Ps[4]*P0.Jbc*cos(th)^2 +
        Ps[3]*P0.m*(Ps[1]*P0.rc*cos(th)-Ps[2]*P0.h*sin(th))^2 +
        Ps[5]*P0.Js
    #
    dLdth =
        -w_psi^2*(
            (
                (Ps[4]*P0.Jbc)+(Ps[3]*P0.m)*((Ps[1]*P0.rc)^2-(Ps[2]*P0.h)^2)
            )*cos(th)*sin(th)-(Ps[1]*P0.rc)*(Ps[2]*P0.h)*(cos(th)^2-sin(th)^2)
        ) - (Ps[3]*P0.m)*(Ps[6]*P0.g)*((Ps[1]*P0.rc)*cos(th) - (Ps[2]*P0.h)*sin(th))
    #
    dp_psi_dth =
        -2*(
            ((Ps[4]*P0.Jbc)+(Ps[3]*P0.m)*((Ps[1]*P0.rc)^2-(Ps[2]*P0.h)^2))*cos(th)*sin(th) +
            (Ps[3]*P0.m)*(Ps[1]*P0.rc)*(Ps[2]*P0.h)*(cos(th)^2-sin(th)^2)
        )*w_psi
    #
...
model_univ_u = FastChain(FastDense(2, 16, tanh), FastDense(16, 2))
...
plot!(sol_nn, vars = (rad2deg2, 0, 1), lw = LW1, lc = LC3, label = L"$\theta$")
plot!(tm, theta*180/pi, lw = LW1, ls = LS2, lc = LC2, label = L"\theta^\mathrm{d}")
plot!(xlim = tspan, xlabel = L"time $t$ [s]", ylabel = L"$\theta$ [$^\circ$]")
plot!(title = "Pitch angle: discovered (blue) vs. data (red) vs NN (green)", box = true)
figname="Helicopter_pitch-angle_model_fit_FNN_u_eq.svg"
...
plot!(title = "Yaw angle: discovered(blue) vs. data (red) vs NN (green)", box = true)
figname="Helicopter_yaw-angle_model_fit_FNN_u_eq.svg"
savefig(figpath*figname)

open("../optimization_results/equation_discovery_u.txt", "w") do f
    println(f, Ψ)
    println(f)
    print_equations(f, Ψ)
    println(f)
